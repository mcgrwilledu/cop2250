...
public class ChiliToGo {
	...
	main(String[] args)
	{
		...
		subtotalAdult = priceForAdultInCents * nadult;
		subtotalChild = priceForChildInCents * nchild;
		subtotalAll   = subtotalAdult + subtotalChild;

		/* closer to expected output, but doesn't print 0 pennies output well */
		...
		System.out.println("Grand Total for all meals is $"
		...
		System.out.println("\tTotal is $"
				...
				+ subtotalChild % 10);
		System.out.println("Grand Total for all meals is $"
				+ (subtotalAdult + subtotalChild) / 100
				+ "."
				+ (subtotalAll % 100) / 10
				+ subtotalAll % 10);
		 */
